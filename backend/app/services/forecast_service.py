...
# ── Fast lookup indexes (built once at startup, O(1) access vs O(N) DataFrame scan) ──
_EDGE_IDX: dict = {}            # (supplier, facility, product) -> row dict
_EDGE_IDX_SUP_FAC: dict = {}   # (supplier, facility) -> list[row dict]
_EDGE_IDX_SUP: dict = {}       # supplier -> list[row dict]
_EDGE_IDX_TYPE_FAC_PROD: dict = {}  # (supplier_type, facility, product) -> list[row dict]
_EDGE_IDX_PROD: dict = {}      # product -> list[row dict]

_ESTATE_IDX: dict = {}         # (supplier, facility) -> row dict
_ESTATE_IDX_SUP: dict = {}     # supplier -> list[row dict]

# Per-request cache for compute_edge_estimated_days (set/cleared in trace_orders_service)
_EDGE_DAYS_CACHE: dict = {}


def _build_edge_indexes(df: "pd.DataFrame") -> None:
    """Build O(1) lookup dicts from EDGE_LEADTIME_MASTER DataFrame."""
...
def compute_edge_estimated_days(
    ...
    # Per-request cache: same supplier+facility+product+qty → same result
    cache_key = (supplier_id, facility_id, product, round(allocated_qty, 2))
    if cache_key in _EDGE_DAYS_CACHE:
        return _EDGE_DAYS_CACHE[cache_key]
    ...
    if supplier_type == "ESTATE" or product == "FFB":
        ...
        if row is not None:
            ...
            _EDGE_DAYS_CACHE[cache_key] = result
            return result
