...
def build_base_sloc_master_from_snapshot(snapshot: pd.DataFrame) -> pd.DataFrame:
    ...
    if not get_app_data("app_data_loaded", False):
        raise RuntimeError(
            "Application data is not fully loaded. "
            "The server may still be initializing. Please wait a moment and try again."
        )
...
def ensure_sloc_config_seeded() -> None:
    ...
    snapshot = load_stock_snapshot(events_bc_slim).copy()
...
def get_cached_sloc_master(
    ...
    if not get_app_data("app_data_loaded", False):
        raise RuntimeError(
            "Application data is not fully loaded. "
            "The server may still be initializing. Please wait and try again."
        )
...
def allocate_stock(
    ...
    # ✅ PERFORMANCE: Filter with .loc to avoid a full-table copy.
    refinery_mask = sloc_state["refinery_group"] == refinery_group
    refinery_pool = sloc_state.loc[refinery_mask]
