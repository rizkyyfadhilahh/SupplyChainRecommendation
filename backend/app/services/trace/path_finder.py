...
def trace_paths_from_facility(
    ...
    depth: int = 0,
) -> Tuple[List[List[Dict[str, Any]]], List[Dict[str, str]], Dict[str, Any]]:
    MAX_DEPTH = 20
    MAX_NODES = 5000

    if depth >= MAX_DEPTH:
        raise HTTPException(status_code=400, detail=f"Traversal aborted: MAX_DEPTH limit of {MAX_DEPTH} exceeded.")
    ...
    trace_meta["nodes_expanded"] = trace_meta.get("nodes_expanded", 0) + 1
    if trace_meta["nodes_expanded"] > MAX_NODES:
        raise HTTPException(status_code=400, detail=f"Traversal aborted: MAX_NODES limit of {MAX_NODES} exceeded.")
