      ...
      <div className="px-6 py-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 border-b border-gray-100 bg-gray-50">
         <div className="flex flex-col gap-1">
           ...
           <select
             ...
             className="border border-gray-200 rounded-lg px-3 py-2.5 text-sm bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
           >
            ...
            {filteredRows.map((row) => {
              ...
              return (
                <tr key={key} className={`border-t border-gray-100 text-gray-700 ${isDirty ? "bg-yellow-50" : "bg-white"}`}>
                  <td className="px-4 py-3 align-top whitespace-nowrap">{row.plant}</td>
                  <td className="px-4 py-3 align-top whitespace-nowrap">{row.name1 || "-"}</td>
                  <td className="px-4 py-3 align-top whitespace-nowrap">{row.storagelocation}</td>
                  <td className="px-4 py-3 align-top whitespace-nowrap">{row.material}</td>
                  <td className="px-4 py-3 align-top whitespace-nowrap">{effectiveProduct}</td>
                  <td className="px-4 py-3 align-top min-w-[220px]">{row.materialdescription || "-"}</td>
                  <td className="px-4 py-3 align-top text-right whitespace-nowrap">{row.current_stock.toLocaleString()}</td>
                  <td className="px-4 py-3 align-top">
                    ...
                    {isDirty && (
                      <div className="mt-3 text-[10px] text-gray-600 bg-blue-50/50 p-2.5 rounded-lg border border-blue-100 w-64 whitespace-normal shadow-sm">
                        <span className="font-bold text-blue-800">Impact Preview</span>
                        <p className="mt-1 leading-relaxed">
                          This SLOC stores <strong>{effectiveProduct}</strong> at <strong>{row.name1 || row.plant}</strong> — toggling EUDR status will affect stock eligibility for EUDR-spec orders.
                        </p>
                        <p className="text-blue-600 font-semibold mt-1.5">
                          Matches {rows.filter(r => r.plant === row.plant && (r.product_code || r.material_type) === effectiveProduct).length} SLOC(s) with this product at this facility.
                        </p>
                      </div>
                    )}
                  </td>
